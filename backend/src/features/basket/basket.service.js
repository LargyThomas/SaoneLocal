// Business logic for basket management
// Handles basket retrieval, product addition and product removal. Basket is always linked to the connected client
...
const getBasket = async (userEmail) => {
    ...
    const items = await connexion.query(`
        SELECT 
            bi.basketItemsId,
            bi.basketItemsQuantity,
            p.productId,
            p.productName,
            p.productPrice,
            (bi.basketItemsQuantity * p.productPrice) AS itemTotalPrice
...
const addToBasket = async (userEmail, { productId, quantity }) => {
    // Verify if product exists and get price
    const product = await connexion.query(
        'SELECT productId, productPrice FROM product WHERE productId = $1',
        [productId]
    )
    ...
    const existing = await connexion.query(
        'SELECT basketItemsId, basketItemsQuantity FROM basket_items WHERE usersId = $1 AND productId = $2',
        [userEmail, productId]
    )
    ...
    if (existing.rows.length > 0) {
        ...
        await connexion.query(
            'UPDATE basket_items SET basketItemsQuantity = $1, basketItemsTotalCost = $2 WHERE basketItemsId = $3',
        ...
        await connexion.query(
            'INSERT INTO basket_items (usersId, productId, basketItemsQuantity, basketItemsTotalCost) VALUES ($1, $2, $3, $4)',
...
const removeFromBasket = async (userEmail, basketItemsId) => {
    // Verify if the item belongs to the user's basket before deleting
    const check = await connexion.query(
        'SELECT basketItemsId FROM basket_items WHERE basketItemsId = $1 AND usersId = $2',
    ...
    await connexion.query(
        'DELETE FROM basket_items WHERE basketItemsId = $1',
        [basketItemsId]
    )
