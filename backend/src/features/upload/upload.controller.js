const { uploadProfilePicture, uploadProductPicture } = require('./upload.service')
...
const uploadCatalogProductPicture = async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ message: 'Aucune image fournie.' })
        }

        const imageUrl = await uploadProductPicture(req.file)

        return res.status(200).json({
            message: 'Image produit envoyée avec succès.',
            imageUrl
        })
    } catch (error) {
        if (error.message === 'S3_BUCKET_MISSING') {
...
module.exports = { uploadUserProfilePicture, uploadCatalogProductPicture }
